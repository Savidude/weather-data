...
function init_daterangepicker() {
    ...
    $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
        ...
        $.ajax({
            url: "/data/stations/health/" + wsid + "?start=" + startTimestamp + "&end=" + endTimestamp,
            type: 'GET',
...
app3.controller('stations', function ($scope, $http) {
    ...
    $http.get(url).then(function (response) {
        if (response.status === 200) {
            ...
            document.getElementById('station-name').innerHTML = stationName + " Health Data";
        }
    });
});
...
function plotWeatherData(weatherData) {
    ...
    var tableBody = document.getElementById('healthData');
    tableBody.innerHTML = '';
    weatherData.forEach (function (data) {
        ...
        var batteryData = document.createElement('td');
        batteryData.innerHTML = data.battery;
        row.appendChild(batteryData);

        var signalData = document.createElement('td');
        signalData.innerHTML = data.signal;
        row.appendChild(signalData);
